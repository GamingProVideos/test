...
RegisterNetEvent('gpv-supercharger:updateClientData', function(stations)
    EVStations = stations
end)
Citizen.CreateThread(function()
lib.hideTextUI()
	for i = 1, #Config.EVStations do
	npc[i] = nil
	end
end)
Citizen.CreateThread(function()
DecorRegister(Config.FuelDecor, 1)
	if Config.Blips == true then
        for k,v in ipairs(Config.EVStations) do
		...
		if not IsEntityPlayingAnim(ped, "weapon@w_sp_jerrycan", "fire", 3) then
            ...
            if refuel_withcan < 100 and fuelingCan.metadata.ammo > 0 then
                ...
                if refuel_withcan > FuelLevel then
					...
					TriggerServerEvent("gpv-supercharger:update_can_level_ox", fuelingCan.metadata.ammo)
                end
...
Citizen.CreateThread(function()
	...
	local wait = 500
        ...
        if closestStation then
            ...
            if dist <= 40.0 then
			if EVStations[closestStation.id] ~= nil then
			petrolcanprice = EVStations[closestStation.id].petrol_can_price
			petrolcanstock = EVStations[closestStation.id].petrol_can_stock
			end
...
local player = PlayerPedId()
local coords = GetEntityCoords(player)
local npc_coord = Config.EVStations[closest_stat].npc_coord.coords
local npcdist = #(coords - npc_coord)
	...
	wait = 5
		...
		if IsControlJustReleased(0,38) then
			...
			if EVStations[closest_stat] == nil then
				lib.showContext('OpenFreeNPCMenu')
			elseif EVStations[closest_stat] ~= nil and EVStations[closest_stat].owner == PlayerData.identifier then
			gas_owner()
			elseif EVStations[closest_stat] ~= nil and EVStations[closest_stat].owner ~= PlayerData.identifier then
			gas_no_owner()
			end
...
function pumpLoop(drawingId, stationId)
    Citizen.CreateThread(function()
        while NearestPump do
			...
			if EVStations[stationId] and EVStations[stationId].capacity > 0 and tonumber(EVStations[stationId].fuel_price) then
				DrawText3Ds(pump.coords.x, pump.coords.y, pump.coords.z+1.5 ,Config.Locales.fuel_menu)
            elseif EVStations[stationId] ~= nil and EVStations[stationId].capacity < 1 then
            DrawText3Ds(pump.coords.x, pump.coords.y, pump.coords.z+1.5 ,"~r~" .. Config.Locales.fuel_menu_nofuel .. "~s~")
			elseif EVStations[stationId] == nil then
				DrawText3Ds(pump.coords.x, pump.coords.y, pump.coords.z+1.5 ,Config.Locales.fuel_menu)
            end
			end
            ...
            if Hose.active then
                ...
                if EVStations[stationId] and EVStations[stationId].capacity then
                    local price = (AmountUsed * EVStations[stationId].fuel_price)
...
function fillingLoop(vehicle, stationId)
    ...
    Citizen.CreateThread(function()
        ...
        Citizen.Wait(500)
        TriggerServerEvent("gpv-supercharger:updateLevel")
        while IsControlPressed(0,38) do
            Citizen.Wait(500)
            PlayerData = ESX.GetPlayerData()
            ...
            if Filling < 100 and PlayerMoney >= math.ceil(Config.Fuel_Price * (AmountUsed + 1)) then
                local add = Config.refuel_speed
                if EVStations[stationId] and EVStations[stationId].capacity then
                    if EVStations[stationId].capacity < add then                    
                ...
                Filling = Filling + add
                AmountUsed = AmountUsed + add
                TriggerServerEvent("gpv-supercharger:updateLevel", stationId, add)
...
function deleteHose(stationId)
    ...
    if AmountUsed > 0 then
        TriggerServerEvent('gpv-supercharger:payForFuel', stationId, AmountUsed)
        AmountUsed = 0
    end
    ...
    options = {
        {
            ...
            onSelect = function(args)
              TriggerServerEvent("gpv-supercharger:buyGasStation", closest_stat)
            end,
            metadata = {
...
function gas_owner()
	if EVStations[closest_stat] ~= nil then
		if EVStations[closest_stat].petrol_can_stock ~= nil then
		stock_can = ((EVStations[closest_stat].petrol_can_stock/EVStations[closest_stat].petrol_can_max_stock)*100)+0.01
		else
		stock_can = 0.01
		end
				if EVStations[closest_stat].price > 0 then
		...
		gas_statione_price = EVStations[closest_stat].price
		now_can_price = EVStations[closest_stat].petrol_can_price
lib.registerContext(
{
    id = 'OpenOwnerNPCMenu',
...
local _maxlevel = false
		stock_can = ((EVStations[closest_stat].petrol_can_stock/EVStations[closest_stat].petrol_can_max_stock)*100)+0.01
		now_fuel_price = EVStations[closest_stat].fuel_price
		now_can_price = EVStations[closest_stat].petrol_can_price
		balance = EVStations[closest_stat].balance
		if EVStations[closest_stat].level ~= #Config.Levels then
		now_level = EVStations[closest_stat].level
		...
		boss_can_stock = EVStations[closest_stat].petrol_can_stock
		boss_fuel_stock = EVStations[closest_stat].capacity
		stocks_fuelstockprog = (((EVStations[closest_stat].capacity/EVStations[closest_stat].max_capacity)*100)+0.01)
		stocks_canstockprog = (((EVStations[closest_stat].petrol_can_stock/EVStations[closest_stat].petrol_can_max_stock)*100)+0.01)
		stocks_fuel = EVStations[closest_stat].capacity
		stocks_can = EVStations[closest_stat].petrol_can_stock
		stocks_fuel_max = EVStations[closest_stat].max_capacity
		stocks_can_max = EVStations[closest_stat].petrol_can_max_stock
		your_free_fuel = EVStations[closest_stat].max_capacity-EVStations[closest_stat].capacity
		your_free_can = EVStations[closest_stat].petrol_can_max_stock-EVStations[closest_stat].petrol_can_stock
		spawn_coords = Config.EVStations[closest_stat].vehicle_coord.coords
		spawn_heading = Config.EVStations[closest_stat].vehicle_coord.heading
lib.registerContext({
    id = 'OpenBossMenu',
    ...
    options = {
        ...
        },
        {
            ...
            onSelect = function(args)
			if EVStations[closest_stat].level ~= #Config.Levels then
			TriggerServerEvent('gpv-supercharger:levelup', closest_stat)
			else
        ...
        },
        {
			...
			local what = "fuel"
			TriggerServerEvent('gpv-supercharger:update_prices', closest_stat, what, newprice)
            end,
			metadata = {
        ...
        },
        {
			...
			if not input then return end
			local newprice = tonumber(input[1])
			TriggerServerEvent('gpv-supercharger:sellStation', closest_stat, newprice)
            end,
        },
        {
			icon = "fa fa-dollar",
        ...
        options = {
        {
			...
			TriggerServerEvent("gpv-supercharger:withdraw", closest_stat, withdraw)
            end,
        },
        {
			icon = "fa fa-dollar",
			...
			TriggerServerEvent("gpv-supercharger:deposit", closest_stat, deposit)
            end,
        },
        }
    },
...
fuelmenu_pric = EVStations[closest_stat].fuel_price
fuelmenu_cap = ((EVStations[closest_stat].capacity/EVStations[closest_stat].max_capacity)*100)+0.01
    ...
    options = {
        ...
        },
        {
			...
			if Config.ox_inventory == true then
				...
				TriggerServerEvent('gpv-supercharger:petrol_can_up', closest_stat)
				else
				...
				if HasPedGotWeapon(player, 883325847) then
					if refillCost > 0 then
						if currentCash >= refillCost then
							TriggerServerEvent('gpv-supercharger:payForFuel', closest_stat, refillamount)
...
local vehicle_mission = GetEntityModel(GetVehiclePedIsIn(player_ped, false))
		...
		wait = 100
			...
			FreezeEntityPosition(GetVehiclePedIsIn(player_ped, false),true)
				...
				if lib.progressBar({
					...
					TriggerServerEvent("gpv-supercharger:balanceremove", station_mission, price_now)
					end
				else
	...
	elseif fuel_task == 2 then
		local distance2 = #(Config.EVStations[station_mission].vehicle_coord.coords - player_coord)
		distance_now = distance2
		if fuel_mission_blip == nil then
	    fuel_mission_blip = AddBlipForCoord(Config.EVStations[station_mission].vehicle_coord.coords.x, Config.EVStations[station_mission].vehicle_coord.coords.y, Config.EVStations[station_mission].vehicle_coord.coords.z)
        SetBlipSprite(fuel_mission_blip, 1)
        SetBlipDisplay(fuel_mission_blip, 4)
        SetBlipScale(fuel_mission_blip, 0.75)
        SetBlipColour(fuel_mission_blip, 5)
        ...
        EndTextCommandSetBlipName(fuel_mission_blip)
		SetNewWaypoint(Config.EVStations[station_mission].vehicle_coord.coords.x,Config.EVStations[station_mission].vehicle_coord.coords.y)
		end
		x = Config.EVStations[station_mission].vehicle_coord.coords.x
		y = Config.EVStations[station_mission].vehicle_coord.coords.y
		z = Config.EVStations[station_mission].vehicle_coord.coords.z
			if distance2 < 4 then
			wait = 100
			if IsControlPressed(0,38) then
			distance_now = 100
			...
			FreezeEntityPosition(GetVehiclePedIsIn(player_ped, false),true)
				...
				if lib.progressBar({
					...
					TriggerServerEvent("gpv-supercharger:missionupdateLevel", station_mission, fuel_order_amount)
					end
				else
				TriggerEvent("ESX:Notify", "error", 3000, Config.Locales.mission_notruck)
				end
...
local vehicle_mission = GetEntityModel(GetVehiclePedIsIn(player_ped, false))
	...
	if can_task == 3 then
		local distance3 = #(Config.EVStations[station_mission].vehicle_coord.coords - player_coord)
		distance_now = distance3
		if can_mission_blip == nil then
	    can_mission_blip = AddBlipForCoord(Config.EVStations[station_mission].vehicle_coord.coords.x, Config.EVStations[station_mission].vehicle_coord.coords.y, Config.EVStations[station_mission].vehicle_coord.coords.z)
        SetBlipSprite(can_mission_blip, 1)
        SetBlipDisplay(can_mission_blip, 4)
        SetBlipScale(can_mission_blip, 0.75)
        SetBlipColour(can_mission_blip, 5)
		...
		if distance3 < 4 then
			...
			wait = 5
				...
				misson_can_text = false
					DeleteVehicle(GetVehiclePedIsIn(player_ped, false))
					TriggerServerEvent("gpv-supercharger:missionupdatecan", station_mission, can_order_amount)
				else
				TriggerEvent("ESX:Notify", "error", 3000, Config.Locales.mission_notruck)
				end
			end
